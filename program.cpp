...
{	
    ...
    while(1)
    {
		...
		cout<<"\n 3 Check List of Buses Registered ";
		cout<<"\n 4 Check List of Passengers Registered";
		cout<<"\n 0. Go Back <- \n";
		int choice;
...
int Admin::registerBus()
{
    ...
    {      int recordFound =0;
	   string line;
        ...
        if(recordFound == 1)
        {
        	cout<<"\n Bus already Register. Please choose another name ";
        	getchar();
        	read.close();
        	return 0;
		}
	...
	out1<<name<<","; 	out1<<num<<",";
	out1<<busCompany<<","; 	out1<<add<<",";
	out1<<from<<",";	out1<<to<<",";
	out1<<distance<<",";
	out1<<fare<<",";	out1<<total_seats<<endl;
	out1.close();
...
void passenger::book_det()
{
    ...
    cout<<"\n*Our services connect Bhubaneswar,Puri,Balasore and Cuttack\n";
  	...
  	{ 
    	...
    	cout<<"  Enter the year:";
     		cin>>depart.year;
    	if(depart.date>=1&&depart.date<=31&&depart.month>=1&&depart.month<=12&&depart.year>=2016&&depart.year<=2020)
      		break;
